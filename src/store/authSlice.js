        ...
        if (refreshToken) {
          try {
            ...
            if (refreshData.refresh) {
              await storage.setRefreshToken(refreshData.refresh);
            }
...
export const login = createAsyncThunk(
  'auth/login',
  async (
    { email, password, rememberMe = true, fcmToken = '' },
    { rejectWithValue }
  ) => {
    try {
      const { authAPI } = await import('../api/auth');
...
export const biometricLogin = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const credentials =
        await storage.getBiometricCredentials();
      if (!credentials) {
        return rejectWithValue(
  ...
  async () => {
    try {
      ...
      return {
        ...
        hasCredentials,
        canUseBiometric:
          hasHardware && isEnrolled && hasCredentials,
      };
    } catch {
      return {
        available: false,
...
export const selectAuthErrors = (state) =>
  state.auth.errors;
export const selectBiometrics = (state) =>
  state.auth.biometrics;
export const selectUserRole = (state) =>
  state.auth.user?.role;
export const selectIsVendor = (state) =>
  state.auth.user?.role === USER_ROLES.VENDOR;
export const selectIsAdmin = (state) =>
  state.auth.user?.role === USER_ROLES.ADMIN;
export const selectIsCustomer = (state) =>
  state.auth.user?.role === USER_ROLES.CUSTOMER;
export const selectRememberMe = (state) =>
  state.auth.rememberMe;
export const selectAccessToken = (state) =>
  state.auth.accessToken;
export const selectVendorStatus = (state) =>
  state.auth.user?.vendor_status;
export const selectIsApprovedVendor = (state) =>
  state.auth.user?.role === USER_ROLES.VENDOR &&
  state.auth.user?.vendor_status === 'approved';
